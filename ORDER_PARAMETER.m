...
function [L_PSI6, L_theta6, neighboridx, G_PSI6, G_C6] = ORDER_PARAMETER(CurrFrame)
...
% distance threshold for OP calculation in the unit of radius
diam = 2010;        % Particle diameter, nm
...
dh = (1+sqrt(3))*aeff/diam*2;  % Upper coordination radius bound, nm
% dh = 2.64;
...
for p1 = 1:pnum
    ...
    % add the self index to the neighbor list and sort the list
    neighboridx{p1} = sort(cat(2,neighboridx{p1},p1)); 
    
    if size(neighboridx{p1},2) > 1
        
        % local theta
        L_theta6(p1) = L_theta6(p1)/(size(neighboridx{p1},2)-1)*180/pi; 
        
        % local psi6
        psii(p1) = psii(p1)/(size(neighboridx{p1},2)-1);
        psir(p1) = psir(p1)/(size(neighboridx{p1},2)-1);
        L_PSI6(p1) = sqrt(psii(p1)^2 + psir(p1)^2);
        
        % real and imaginary part of global psi6
